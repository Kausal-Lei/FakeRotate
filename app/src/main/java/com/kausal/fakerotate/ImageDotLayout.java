...
public class ImageDotLayout extends FrameLayout implements View.OnClickListener, View.OnLongClickListener {
            ...
            public void onMatrixChanged(RectF rectF) {
                if (iconList != null && iconList.size() > 0) {
                    for (ImageView icon : iconList) {
                        ...
                        //保持底部中心位置不变
                        icon.setX(rectF.left + newX - DensityUtil.dp2px(getContext(), 24) / 2);
                        icon.setY(rectF.top + newY - DensityUtil.dp2px(getContext(), 24)/2);
                    }
                }
                tempRectF = rectF;
                //图片加载完成后才可以添加图标
    ...
    public void addIcon(IconBean bean) {
        ...
        LayoutParams layoutParams = new LayoutParams(DensityUtil.dp2px(getContext(), 24), DensityUtil.dp2px(getContext(), 24));
